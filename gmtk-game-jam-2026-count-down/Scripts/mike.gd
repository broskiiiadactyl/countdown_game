...
var resource := "res://Dialogue/mike.dialogue"

@onready var anim_player : AnimationPlayer = %Model.get_node("%AnimationPlayer2")


func _ready() -> void:
	DialogueManager.dialogue_ended.connect(test)
	
	anim_player.play("mike/IdleAnim")
